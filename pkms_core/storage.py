...
class SqliteTaskStore(TaskStore):
    ...
    def _ensure_schema(self):
        with self._conn() as conn:
            conn.execute("""CREATE TABLE IF NOT EXISTS tasks (id INTEGER PRIMARY KEY, text TEXT, created TEXT, completed INTEGER)""")
    def load(self) -> List[Task]:
        with self._conn() as conn:
    ...
    def save_all(self, tasks: List[Task]) -> None:
        with self._conn() as conn:
            ...
            for t in tasks:
                conn.execute("INSERT INTO tasks(id,text,created,completed) VALUES(?,?,?,?)", (t.id,t.text,t.created,int(t.completed)))
    def add(self, task: Task) -> None:
        with self._conn() as conn:
            conn.execute("INSERT INTO tasks(id,text,created,completed) VALUES(?,?,?,?)", (task.id,task.text,task.created,int(task.completed)))
    def update(self, task: Task) -> None:
        with self._conn() as conn:
            conn.execute("UPDATE tasks SET text=?, created=?, completed=? WHERE id=?", (task.text,task.created,int(task.completed),task.id))
    def delete(self, task_id: int) -> bool:
        with self._conn() as conn:
            cur = conn.execute("DELETE FROM tasks WHERE id=?", (task_id,))
            return cur.rowcount>0

# Document storage simple JSON only for prototype
class DocumentStore:
    def __init__(self, path: str):
        self.path = path
...
def make_task_store(kind: str, base_dir: str) -> TaskStore:
    ...
    if kind == 'sqlite': return SqliteTaskStore(os.path.join(data_dir,'tasks.db'))
